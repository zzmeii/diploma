...
def mnd():
    ...
    for i in range(len(dir_names)):
        dir_names[i] = int(dir_names[i])
    p_name = f'results\\{0 if len(dir_names) == 0 else max(dir_names) + 1}'
    os.mkdir(p_name)
...
class GPTree:
    def __init__(self, data=None, left=None, right=None):
        self.data = data
        self.left: GPTree = left
        self.right: GPTree = right
    ...
    def draw_tree(self, fname, footer):
        ...
        count = [0]
        self.draw(dot, count)
        Source(dot[0], filename=p_name + '\\' + fname + ".gv", format="png").render()
        display(Image(filename=p_name + '\\' + fname + ".gv.png"))
    ...
    def compute_tree(self, x):
        if self.data in FUNCTIONS:
            ...
            return x[TERMINALS.index(self.data)]
        else:
            return self.data
...
def convert_to_string(tree):
    ...
    if tree.data in ONE_BRANCH_FUNC:
        return f'{names[tree.data.__name__]}({convert_to_string(tree.right)})'
    if tree.data in FUNCTIONS:
        if type(convert_to_string(tree.right)) == int and type(convert_to_string(tree.left)) == int:
            return tree.data(convert_to_string(tree.left), convert_to_string(tree.right))
        return f'({convert_to_string(tree.left)}{names[tree.data.__name__]}{convert_to_string(tree.right)})'
    return tree.data
...
def fitness(individual, dataset, bloat_control, error=error):
    if bloat_control:
        ...
        return error(individual, dataset)
...
def symbol_regression(dataset, pop_size=100, min_depth=4, max_depth=5,
                      generations=250, tournament_size=5, xo_rate=0.8,
                      prob_mutation=0.35, bloat_control=False, add_x=True, error=error, headers=None):
    mnd()
    global p_name
    printProgressBar(0, generations, prefix='Progress:', suffix='Complete', length=50)
    conf = {
        'pop_size':        pop_size,
        'min_depth':       min_depth,
        'max_depth':       max_depth,
        ...
        'bloat_control':   bloat_control,
        'add_x':           add_x,
        'error':           error
    }
    conf_file = open(p_name + r'\config.txt', 'x')
    for i in conf:
        if i == 'error':
            continue
        conf_file.write(f'{i}: {conf[i]}\n')
    ...
    for gen in range(generations):
        ...
        printProgressBar(counter, generations, prefix='Progress:', suffix='Complete', length=50)
    ...
    plt.savefig(p_name + r'\err_and_size.png')
    conf_file.write("function: " + convert_to_string(best_of_run) + '\n')
    conf_file.close()
    return best_of_run
